...
{
	int addr_set = (addr >> b) & set_index_mask;
	int addr_tag = (addr >> (b+s));
	
	//Hit Check
	for(int i = 0; i<E; i++){
		if (cache[addr_set][i].tag == addr_tag && cache[addr_set][i].valid){
			hit_count++;
			mru_counter++;
			cache[addr_set][i].mru = mru_counter;
	...
	//Last Resort: Eviction Check
	eviction_count++;
	if(verbosity) printf(" eviction ");
	int recent = 0;
	for(int i = 0; i<E; i++){
		if(cache[addr_set][i].valid &&
		 cache[addr_set][i].mru >= cache[addr_set][recent].mru){
			recent = i;
		}
	}
	mru_counter++;
	cache[addr_set][recent].valid = 1;
	cache[addr_set][recent].tag = addr_tag;
	cache[addr_set][recent].mru = mru_counter;
}
...
void replayTrace(char* trace_fn)
{
    ...
    while( fgets(buf, 1000, trace_fp) != NULL) {
		...
		com = buf[1];
		if(verbosity && com != ' ') printf("%c %llx,%u", com, addr, len);
		if(com == 'L' || com == 'S') {
			accessData(addr);
		} else if(com == 'M'){
			accessData(addr);
