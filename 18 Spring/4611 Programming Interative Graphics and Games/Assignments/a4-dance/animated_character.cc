...
Matrix4 AnimatedCharacter::rootTransform(const Matrix4 &model_matrix){
    return model_matrix * accum_translation_matrix_ * pose_.root_rotation();
}
                                          ...
                                          const Matrix4 &view_matrix, const Matrix4 &proj_matrix)
{
    ...
    /**You will need to define a current transformation matrix for this bone that takes into account not just the parent_transform but also 
     * the local rotation of the bone due to the current pose. Think of the vertices that make up the geometry of each bone as being defined in "bone space", 
     * where the joint that the bone rotates around is located at the origin and the bone extends in the direction and length specified by the skeleton. 
     * (See Skeleton::BoneDirectionAndLength()).To determine which matrices need to be composed to create the current transformation matrix and the order to 
     * multiply them together, think about what needs to happen to each vertex of a cylinder defined in "bone space" in order to get the vertex to the correct 
     * position in 3D space.
     * 
     * First, the vertex must be transformed into the bone's "rotation axis space" because the rotation axes are not guaranteed to line up 
     * perfectly with the bone's X,Y,Z axes.  The bone's rotation axes are a property of the skeleton -- they are set for each skeleton and do not change for each 
     * pose.  You can access a matrix that transforms from "bone space" to "rotation axis space" from the skeleton_ member variable.
     * 
     * Second, now that the vertices are in the bone's "rotation axis space", the rotation from the character's current pose can be applied.  
     * The current pose is stored in the pose_ member variable. 
     * 
     * Third, with the rotations applied relative to the appropriate rotation axes, the vertices must now be transformed back into regular 
     * "bone space".  At this point, the bone should be properly rotated based upon the current pose, but the vertices are still defined in "bone space" so 
     * they are close to the origin.
     * 
    ...
    **/
    ...
    if (bone_name == "lowerback") {
        quick_shapes_.DrawSphere(ctm*Matrix4::RotationY(GfxMath::ToRadians(270))*Matrix4::RotationZ(GfxMath::ToRadians(60))*Matrix4::Translation(Vector3(-0.2,-0.09,0))*Matrix4::Scale(Vector3(2,1,1.5))*S, view_matrix, proj_matrix, ant);
    }
    if (bone_name == "upperback") {
        quick_shapes_.DrawSphere(ctm*Matrix4::Scale(Vector3(0.14,0.1,0.1)), view_matrix, proj_matrix, ant);
    }
    if (bone_name == "thorax") {
        quick_shapes_.DrawSphere(ctm*Matrix4::Translation(Vector3(0,0.07,0))*Matrix4::Scale(Vector3(0.14,0.1,0.1)), view_matrix, proj_matrix, ant);
    }
    ...
    if (bone_name == "head") {
        quick_shapes_.DrawSphere(ctm*Matrix4::Translation(Vector3(0,0.05,0.05))*Matrix4::RotationX(GfxMath::ToRadians(45))*Matrix4::Scale(Vector3(0.12,0.1,0.2)), view_matrix, proj_matrix, ant);
        quick_shapes_.DrawSphere(ctm*Matrix4::Translation(Vector3( 0.04,0.135,0.07))*Matrix4::Scale(Vector3(0.025,0.025,0.025)), view_matrix, proj_matrix, Color(0,0,0));
        quick_shapes_.DrawSphere(ctm*Matrix4::Translation(Vector3(-0.04,0.135,0.07))*Matrix4::Scale(Vector3(0.025,0.025,0.025)), view_matrix, proj_matrix, Color(0,0,0));
        //quick_shapes_.DrawCube(ctm*Matrix4::Translation(Vector3(0,0.-0.01,0.25))*Matrix4::RotationX(GfxMath::ToRadians(60))*Matrix4::Scale(Vector3(0.05,0.025,0.01)), view_matrix, proj_matrix,Color(0,0,0));
        quick_shapes_.DrawCone(ctm*
                    Matrix4::RotationX(GfxMath::ToRadians(10))*
                    Matrix4::RotationZ(GfxMath::ToRadians(-20))*
                    Matrix4::Translation(Vector3( 0.02,0.24,-0.04))*
                    Matrix4::Scale(Vector3(0.02,0.15,0.02)), 
                    view_matrix, proj_matrix, ant);
        quick_shapes_.DrawCone(ctm*
                    Matrix4::Translation(Vector3(0.14,0.34,0.07))*
                    Matrix4::RotationX(GfxMath::ToRadians(90))*
                    Matrix4::Scale(Vector3(0.02,0.07,0.02)), 
                    view_matrix, proj_matrix, ant);
    ...
    /**
    // Determining the proper child_root_transform is the key here.  It depends on the
    ...
    **/
    ...
    quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, ant, Point3(0,0,0), Point3(0,0,0)+skeleton_.BoneDirectionAndLength(bone_name), 0.018 );
