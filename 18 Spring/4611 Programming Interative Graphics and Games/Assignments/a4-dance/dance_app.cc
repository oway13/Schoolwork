...
DanceApp::DanceApp() : GraphicsApp(1280,768, "So You Think Ants Can Dance") {
    ...
    ballet_special2_.LoadFromAMC(Platform::FindFile("32_20.amc", searchPath_), *ballet_ant_.skeleton_ptr());
    ballet_special2_.TrimFront(1100);
    ballet_special2_.TrimBack(60);
    ballet_special2_.CalcRelativeTranslations();
    
    ballet_special3_.LoadFromAMC(Platform::FindFile("54_12.amc", searchPath_), *ballet_ant_.skeleton_ptr());
    ballet_special3_.TrimFront(30);
    ballet_special3_.TrimBack(30);
    ballet_special3_.CalcRelativeTranslations();
    
    ballet_special4_.LoadFromAMC(Platform::FindFile("85_04.amc", searchPath_), *ballet_ant_.skeleton_ptr());
    ballet_special4_.TrimFront(240);
    ballet_special4_.TrimBack(30);
    ballet_special4_.CalcRelativeTranslations();
    
    ballet_special5_.LoadFromAMC(Platform::FindFile("61_10.amc", searchPath_), *ballet_ant_.skeleton_ptr());
    ballet_special5_.TrimFront(120);
    ballet_special5_.TrimBack(0);
...
void DanceApp::InitOpenGL() {
    ...
    bg_tex_.InitFromFile(Platform::FindFile("ants-dance2.png", searchPath_));
...
void DanceApp::OnMotion2BtnPressed() {
    ballet_ant_.OverlayClip(ballet_special2_, 240);
}
...
void DanceApp::OnMotion5BtnPressed() {
    ballet_ant_.OverlayClip(ballet_special5_, 200);
}

void DanceApp::OnSpecialKeyDown(int key, int scancode, int modifiers){
...
void DanceApp::DrawUsingOpenGL() {
    ...
    if(onBallet){
        Point3 bant = ballet_ant_.rootTransform(ballet_ant_transform_)*Point3(0,0,0);
        viewMatrix_ = Matrix4::LookAt(Point3(0,1.5,3.5), bant, Vector3(0,1,0));
    } else{
        viewMatrix_ = Matrix4::LookAt(Point3(0,1.5,3.5), Point3(0,1,0), Vector3(0,1,0));
    }
