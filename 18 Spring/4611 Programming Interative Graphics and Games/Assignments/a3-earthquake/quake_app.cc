...
void QuakeApp::OnGlobeBtnPressed() {
    // This is where you can switch between flat earth mode and globe mode
	// Checkmate Globies
	rotationY = 0;
	rotationX = 0;
	transitioning = true;
...
void QuakeApp::UpdateSimulation(double dt)  {
	...
	if (transitioning) {
		//cout << "transitioning" << trans_frac << "\n";
		float delta_frac = dt / 1; //I think this should make it take 1000 ms
		trans_frac = GfxMath::Clamp((trans_frac += delta_frac), 0, 1);
		if (plane) { //Plane to Globe
			earth_.ToGlobe(trans_frac);
		} else { // Globe to Plane
			earth_.ToPlane(trans_frac);
		}
		if (trans_frac == 1) { //If we're done 
			transitioning = false;
			trans_frac = 0;
			plane = !plane;
		}
...
void QuakeApp::DrawUsingOpenGL() {
    ...
    Matrix4 model_matrix = 
						Matrix4::RotationX(GfxMath::ToRadians(rotationX)) *
						Matrix4::RotationY(GfxMath::ToRadians(rotationY));
    ...
    // Draw the earthquakes. Getting earthquakes from the past year, using the code from the discussion
	// Drawing the earthquakes as spheres scaled by magnitude
	...
	const float eq_scale = 15.0;

	for (int x = start; x<end; x++) {

		Earthquake e = quake_db_.earthquake(x);

		double lati = e.latitude();

		double longi = e.longitude();

		float sig = 0.1 + (1-0.1)*(e.magnitude() - quake_db_.min_magnitude()) / (quake_db_.max_magnitude() - quake_db_.min_magnitude());

		Color ballcol(1.0, 0.0, 0.0, 0.5);
		Matrix4 Mball;
		//Plane
		if (plane) {
			Mball =
				Matrix4::Translation(earth_.LatLongToPlane(lati, longi) - Point3(0, 0, 0)) *
				Matrix4::Scale(Vector3(sig / eq_scale, sig / eq_scale, sig / eq_scale)); 
		}
		//Globe
		else {
			Mball =
				Matrix4::Translation(earth_.LatLongToSphere(lati, longi) - Point3(0, 0, 0)) *
				Matrix4::Scale(Vector3(sig / eq_scale, sig / eq_scale, sig / eq_scale));
		}
