...
#ifndef INCLUDE_EXT_TOKEN_H_
#define INCLUDE_EXT_TOKEN_H_
 ...
 * Class Definitions
 ******************************************************************************/
/*! Extends a Token object by adding properties useful for parsing and
 *  describing the Token.
 */
class ExtToken {
 public:
  ExtToken(parser::Parser *p, Token *t)
      : desc_str_(), lexeme_(t->get_lexeme()), terminal_(t->get_terminal()),
                                                      next_(NULL), parser_(p) {}
  ExtToken(parser::Parser *p, Token *t, std::string d)
      : desc_str_(d), lexeme_(t->get_lexeme()),
        terminal_(t->get_terminal()),
        parser_(p) {}
