...
ExtToken *ExtToken::ExtendToken(parser::Parser *p, Token *tokens) {
  switch (tokens->get_terminal()) {
    ...
    case kNotEquals:
      return new RelationalOpToken(p, tokens, tokens->get_lexeme());
    ...
    default:
      printf("%i not implemented extend", tokens->get_terminal());
      fflush(stdout);
      assert(0);
  } /* switch() */
